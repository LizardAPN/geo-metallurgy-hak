...
(:Publication {doc_id, title, year, lang, doc_type, venue}) — у Publication НЕТ поля geography. Географию фильтруй ТОЛЬКО по полю geography сущностей (Process/Experiment/Facility/Expert).
...
4. Фильтры контекста, если заданы: год — через (x)-[:DESCRIBED_IN]->(p:Publication) WHERE p.year >= .. AND p.year <= ..; география — n.geography CONTAINS '...' (никогда строгим равенством: 'RU' может быть частью 'RU,KZ').
...
10. Термины процессов ищи на узлах Process и через связи: эксперимент по процессу — это (exp:Experiment)-[]-(proc:Process) WHERE proc.name_norm CONTAINS ..., а не подстрока в имени эксперимента. Связь Experiment—Process указывай ненаправленной и нетипизированной: (exp:Experiment)-[]-(proc:Process). Не придумывай направление и тип связи. Проверка aliases — всегда any(a IN n.aliases WHERE toLower(a) CONTAINS '...'), никогда aliases CONTAINS 'слово'.
11. geography сравнивай через CONTAINS ('RU' может быть частью 'RU,KZ'), никогда строгим равенством (= 'RU').
12. Пользовательское ограничение (порог/диапазон) должно матчить И точечные факты (operator '='). Для условия '<= X' пиши: ((r.value <= X) OR (r.operator = 'range' AND r.value_min <= X)) — БЕЗ проверки operator у точечных значений. Для диапазона [A,B]: ((r.value >= A AND r.value <= B) OR (r.operator = 'range' AND r.value_min <= B AND r.value_max >= A)).
13. Не требуй все термины вопроса в name_norm одного узла конъюнкцией. Разные аспекты (процесс/вещество/среда) распределяй по связанным узлам: процесс 'очистка сточных вод' + через связь Property/Material с 'сульфат'. Максимум 2 CONTAINS-условия AND на один узел.
...
{"cypher": "MATCH (exp:Experiment)-[]-(proc:Process) WHERE toLower(proc.name_norm) CONTAINS 'кучн' AND toLower(proc.name_norm) CONTAINS 'выщелач' MATCH (exp)-[]-(m:Material) WHERE m.name_norm CONTAINS 'никел' OR any(a IN m.aliases WHERE toLower(a) CONTAINS 'nickel' OR toLower(a) CONTAINS 'никел') MATCH (exp)-[:DESCRIBED_IN]->(pub:Publication) WHERE pub.year >= 2015 AND exp.geography CONTAINS 'RU' RETURN labels(exp)[0] AS type, exp.name AS name, proc.name AS process, m.name AS material, pub.doc_id AS doc_id, pub.title AS title, pub.year AS year LIMIT 50", "explanation": "Эксперименты по кучному выщелачиванию никелевых руд; год и география из вопроса; материал — никель; proc и exp не обязаны в одной публикации"}
"""
